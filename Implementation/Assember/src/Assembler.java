...
class Memory
{
  ...
  public String dump()   //Dump the content of current Memory object as the form of 34-bit machine code.
  {
    ...
    String[] ones={"","1","11","111","1111","11111","111111","1111111","11111111","111111111"};
    for(j=0;j<size;j++)
    {
    	String tempOutput;
    	if (entries[j].data.startsWith("0x")) {
        tempOutput = entries[j].data.substring(entries[j].data.lastIndexOf("0x")+2,entries[j].data.length());
           if(j<size-1)
           ...
           output+=zeros[9 - tempOutput.length()]+tempOutput;
    			...
    			tempOutput = entries[j].data;
    			//System.out.println(tempOutput);
    			//tempOutput 
    			//int memoint = Integer.parseInt(entries[j].data, 16);
    			//System.out.println(memoint);
    			if(j<size-1 ) {
    				if (tempOutput.charAt(0) == '0')
    					output+=zeros[9 - tempOutput.length()]+tempOutput+",\n";
    				else 
    					output+=ones[9 - tempOutput.length()]+tempOutput+",\n";
    			}
    		    else {
    		    	if (tempOutput.charAt(0) == '0')
   					 	output+=zeros[9 - tempOutput.length()]+tempOutput;
...
//	public static void main(String[] args){
	//	Instruction instruc = new Instruction("add $a0 9($a1) 0x8");
//		instruc.print();
  ...
  public int lalala = 0; //lalala = 1 stands for 'la' struction
  public int la_i = 0;
  public int la_j = 0;
  public char[] la_char;
  public char[] la_sixdigit = new char[6];
  //public String newla_string;
  ...
  { //The most important method in Assembler class. This function controls the flow of Assembler. You may not need to change it.
    ...
    while(sourceCodeLine != null)
    {
      ...
      else if(isLabel(sourceCodeLine))
      {
        ...
        if(label != null)
        { //System.out.println(label+dataMemoryAddress);
          processLabel(label);
        } 
        else
    ...
    // output code
    //System.out.println(instructions[0].operator+ " " +instructions[0].operands[0].name+" " +instructions[0].operands[0].offset
    //		+ " " +instructions[0].operands[1].name+" " +instructions[0].operands[1].offset);
  //  System.out.println(instructions[1].operator+ " " +instructions[1].operands[0].name+" " +instructions[1].operands[0].offset
    //		+ " " +instructions[1].operands[1].name+" " +instructions[1].operands[1].offset); 

    out_code.write("MEMORY_INITIALIZATION_RADIX=16;\nMEMORY_INITIALIZATION_VECTOR=\n");
    for(int i=0; i < instructionCount; i++)
    {
      currentinstructionCount = i;
