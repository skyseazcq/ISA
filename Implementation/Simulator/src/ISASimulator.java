...
public class ISASimulator {
  ...
  // TODO: modify this
  public static final int REG_FILE_SIZE = 17;
  public static final int OPCODE_LENGTH = 7;// 5;
  ...
  // !!!inst_mem stores the corresponding binary machine code.
  private Int34[] data_mem;
  // !!! data_mem stores decimal value of current memory address data.
  ...
  public int twoCompint(String signedimmlabel){
	 // System.out.println("signedimmlabel: "+signedimmlabel);
	  int intimmlabel;
	  if(signedimmlabel.charAt(0)== '0') {
		  intimmlabel = Integer.valueOf(signedimmlabel,2).intValue();
        }
        // if negative, we first need to do twos complement
          ...
          for(int i = signedimmlabel.length()-1; i >= 0; --i) {
            if(!flip) {
              twos_comp = signedimmlabel.charAt(i) + twos_comp;
              if(signedimmlabel.charAt(i) == '1') flip = true; // start flipping from now on
            }
            else if(signedimmlabel.charAt(i) == '0') twos_comp = '1' + twos_comp;
          ...
          intimmlabel = Integer.valueOf(twos_comp,2).intValue();
        }
	 // System.out.println("intimmlabel: "+intimmlabel);
	  return intimmlabel;
  }
  // default constructor
  public ISASimulator() { 
    ...
    try {
      ...
      while(line != null) { // stop when we read EOF
        ...
        if(imem) {
          ...
          { // need to convert each digit to binary string first
          ...
          Int34 line_value; //!!stores decimal value of current memory address data.
  ...
  public void printDMem() { printDMem(0, D_MEM_SIZE-1); }
  ...
  public void execute(int num_insts) {
   ...
   // String function_str;
    ...
    while(num_done < num_insts) {
      ...
      // TODO: complete this    //include PC operation
      switch (opcode) {
        case 0: //add, sub, nor
        	rsd = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+4),2).intValue();
        	rt = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+4,OPCODE_LENGTH+8),2).intValue();
        	function =  Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+8,OPCODE_LENGTH+10),2).intValue();
        	//System.out.println(function);
        	switch (function){
        	case 0: reg_file[rsd] = reg_file[rsd].add(reg_file[rt]);System.out.println(PC+ ": "+"add..."+"REG"+rsd+", "+"REG"+rt);break;
        	case 1: reg_file[rsd] = reg_file[rsd].subtract(reg_file[rt]);System.out.println(PC+ ": "+"sub..."+"REG"+rsd+", "+"REG"+rt); break;
        	case 2: reg_file[rsd] = (reg_file[rsd].or(reg_file[rt])).not();System.out.println(PC+ ": "+"nor..."+"REG"+rsd+", "+"REG"+rt); break;
        	default: 
        		System.err.println("function number within[0,2], number 3 remained!" );return;
        	}
        	PC = PC + 1;
        	//System.out.println(reg_file[rsd]);
          break;
        case 1: //in, out 
        	rd = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+4),2).intValue();
        	rc = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+4,OPCODE_LENGTH+8),2).intValue();
        	function =  Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+8,OPCODE_LENGTH+10),2).intValue();
        	//channelNostr = hexToBin(Long.toHexString(reg_file[rc].longValue()).charAt(0));
       	 	//channelNo = Integer.valueOf(channelNostr,2).intValue();
        	channelNo = (int)(reg_file[rc].longValue());
       	 	//System.out.println(PC+": "+function+"!channelNo:"+channelNo);
        	switch (function){
        	case 0: reg_file[rd] = channels.get(channelNo).poll();System.out.println(PC+ ": "+"in..."+"REG"+rd+", "+channelNo); break;
        	case 1: channels.get(channelNo).offer(reg_file[rd]);System.out.println(PC+ ": "+"out..."+"REG"+rd+", "+channelNo); break;
        	default: 
        		System.err.println("function number within[0,1], number 2 and 3 remained!" );return;
        	}
        	PC = PC + 1;
          break;
        case 2: //addi
        	imm = twoCompint(curr_inst.substring(OPCODE_LENGTH+4,OPCODE_LENGTH+10));
        	rsd = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+4),2).intValue();
        	imm34 = new Int34(imm);
        	// imm34 = twosCompValue(curr_inst.substring(OPCODE_LENGTH+4,OPCODE_LENGTH+10));
        	reg_file[rsd] = reg_file[rsd].add(imm34);
        	System.out.println(PC+ ": "+"addi..."+ "REG"+rsd + ", "+"0x"+imm34);
        	PC = PC + 1;
          break;
        case 3: //srl  //maybe something wrong
        	rsd = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+4),2).intValue();
        	imm = twoCompint(curr_inst.substring(OPCODE_LENGTH+4,OPCODE_LENGTH+10));
        	if(imm >= 0) reg_file[rsd] = reg_file[rsd].shiftRight(imm);
        	else reg_file[rsd] = reg_file[rsd].shiftLeft((0-imm));
        	System.out.println(PC+ ": "+"srl..."+"REG"+ rsd + ", "+imm);
        	PC = PC + 1;
          break;
        case 4: //sloi
        	rsd = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+4),2).intValue();
        	imm = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+4,OPCODE_LENGTH+10),2).intValue();
        	imm34 = new Int34(imm);
        	reg_file[rsd] =reg_file[rsd].shiftLeft(6).add(imm34);
        	System.out.println(PC+ ": "+"sloi..."+ "REG"+rsd + ", "+"0x"+imm34);
        	PC = PC + 1;
          break;
        case 5: //0101 move
        	rd = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+4),2).intValue();
        	rt = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+4,OPCODE_LENGTH+8),2).intValue();
        	imm =  Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+8,OPCODE_LENGTH+10),2).intValue();
        	imm34 = new Int34(imm);
        	reg_file[rd] =reg_file[rt].add(imm34);
        	System.out.println(PC+ ": "+"move..."+ "REG"+rd + ", "+imm34.longValue()+"("+"REG"+rt+")");
        	PC = PC + 1;
          break;
        case 6: //0110 lw
        	rd = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+4),2).intValue();
        	...
        	imm =  Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+8,OPCODE_LENGTH+10),2).intValue();
        	imm34 = new Int34(imm);
        	reg_file[rd] = data_mem[(int)(reg_file[rt].add(imm34)).longValue()];
        	System.out.println(PC+ ": "+"lw..."+ "REG"+rd + ", "+imm34.longValue()+"("+"REG"+rt+")");
        	PC = PC + 1;
          break;
        case 7: //0111 sw
        	rt = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+4),2).intValue();
        	rd = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+4,OPCODE_LENGTH+8),2).intValue();
        	imm =  Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+8,OPCODE_LENGTH+10),2).intValue();
        	imm34 = new Int34(imm);
        	data_mem[(int)(reg_file[rd].add(imm34)).longValue()] = reg_file[rt];
        	System.out.println(PC+ ": "+"sw..."+ "REG"+rt + ", "+imm34.longValue()+"("+"REG"+rd+")");
        	PC = PC + 1;
          break;
        case 8: //1000 be
        	labeloffset = twoCompint(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+8));
        	imm =  Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+8,OPCODE_LENGTH+10),2).intValue();
         	imm34 = new Int34(imm);
         	System.out.println(PC+ ": "+"be..."+imm34.longValue()+"("+labeloffset+")");
        	if (reg_file[12].equals(reg_file[13]))  PC = PC + labeloffset;
        	else  {PC = PC +1; reg_file[13] = reg_file[13].add(imm34);}
          break;
        case 9: //1001 blt
        	labeloffset = twoCompint(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+8));
        	imm =  Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+8,OPCODE_LENGTH+10),2).intValue();
         	imm34 = new Int34(imm);
         	System.out.println(PC+ ": "+"blt..."+imm34.longValue()+" "+labeloffset+")" );
        	if ((reg_file[12].subtract(reg_file[13])).signum() == -1) PC = PC + labeloffset; // $bcl < $bcr
        	else  {PC = PC +1; reg_file[13] = reg_file[13].add(imm34);}
          break;
        case 10: //1010 jmp 00 jal 01
        	labeloffset = twoCompint(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+8));
        	function =  Integer.valueOf(curr_inst.substring(OPCODE_LENGTH+8,OPCODE_LENGTH+10),2).intValue();
        	//System.out.println(function);
        	switch (function){
        	case 0: PC = PC +labeloffset; System.out.println((PC-labeloffset)+ ": "+"jmp..."+labeloffset);break;
        	case 1: reg_file[11] = new Int34(PC + 1); PC = PC +labeloffset;System.out.println((PC-labeloffset)+ ": "+"jal..."+labeloffset); break; //$ra = PC + 1
        	default: 
        		System.err.println("function number within[0,1], number 2 and 3 remained!" );return;
        	}
          break;
        case 11: //1011 jr
        	rd = Integer.valueOf(curr_inst.substring(OPCODE_LENGTH,OPCODE_LENGTH+4),2).intValue();
        	System.out.println(PC+ ": "+"jr..."+"REG"+rd);
  ...
  public void run() {
    ...
    while(true) {
      ...
      if(curr_token.equals("iload")) {   //iload $i_file.coe $start_addr
      ...
      else if(curr_token.equals("dload")) { //dload $d_file.coe $start_addr
      ...
      else if(curr_token.equals("go")) { //go $number
      ...
      else if(curr_token.equals("dump_reg")) { //dump_reg// prints values in all registers
        printRegFile();
      }
      else if(curr_token.equals("set_reg")) { //set_reg $reg_num $value
      ...
      else if(curr_token.equals("dump_imem")) { //dump_imem $addr $size //disassembles instructions in the instruction memory from $addr to $addr + $size, 
    	 // which might not be exactly the same with the assembly instruction appeared in your assembly file
      ...
      else if(curr_token.equals("set_imem")) { //set_imem $addr $value
      ...
      else if(curr_token.equals("dump_dmem")) { //dump_dmem $addr $size
        if(input_tokens.countTokens() != 2) System.out.println("usage: dump_dmem $start_addr $range");
        else {
          int start_addr = Integer.parseInt(input_tokens.nextToken());
          int range = Integer.parseInt(input_tokens.nextToken());
          printDMem(start_addr,range);   // prints data memory values from $addr to $addr + $size
        }
      }
      else if(curr_token.equals("set_dmem")) {  //set_dmem $addr $value
      ...
      else if(curr_token.equals("dump_channel")) {  //dump_channel $channel
        if(input_tokens.countTokens() != 1) System.out.println("usage: dump_channel $chan_number");
          ...
          printChannel(chan_num);   //prints values put on channel $channel by out instruction
        }
      }
      else if(curr_token.equals("put_channel")) {   //put_channel $channel $value
        if(input_tokens.countTokens() != 2) System.out.println("usage: put_channel $chan_number $value");
        else {
          int chan_num = Integer.parseInt(input_tokens.nextToken());
          Int34 value = new Int34(Long.parseLong(input_tokens.nextToken()));
          addToChannel(chan_num,value);  //puts $value on channel $channel so that in instruction can get a value
        }
      }
      else if(curr_token.equals("clear_channel")) {  //clear_channel $channel
        if(input_tokens.countTokens() != 1) System.out.println("usage: clear_channel $chan_number");
        else {
          int chan_num = Integer.parseInt(input_tokens.nextToken());
          clearChannel(chan_num); //discards all values written to channel $channel so that out instruction can proceed
        }
      }
      else if(curr_token.equals("set_buf_size")) {//set_buf_size $value
        if(input_tokens.countTokens() != 1) System.out.println("usage: set_buf_size $size");
        else {
          int size = Integer.parseInt(input_tokens.nextToken());
          setBufferSize(size); //changes the buffer size for each channel to $value. the initial size is 32
        }                      //while in this frame, it is 16
      }
